...
public class TestDataRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        // 테스트 User 생성 (이미 존재하는지 확인 후 생성)
        User testUser = userRepository.findByEmail("robbie@sparta.com").orElse(null);
        if (testUser == null) {
            testUser = new User("Robbie", passwordEncoder.encode("1234"), "robbie@sparta.com", UserRoleEnum.USER);
            testUser = userRepository.save(testUser);
        }

        // 테스트 User 의 관심상품 등록 (이미 데이터가 있는지 확인 후 생성)
        // 검색어 당 관심상품 10개 등록
        if (productRepository.countByUser(testUser) == 0) {
